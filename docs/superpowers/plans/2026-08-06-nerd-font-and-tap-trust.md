...
- **Corrected during implementation** (see ledger): the tap-trust confirmation prompt (`ui_confirm`) is skipped when `MACUP_NONINTERACTIVE=1` (i.e. under `--all` or named-module runs) — it logs a warning and continues instead of prompting. `--all` is explicitly documented as "non-interactive" in both `bin/macup --help` and the README; a `gum confirm` call under a truly scripted/non-tty `--all` invocation could hang indefinitely waiting for input that will never arrive. This plan originally specified "always prompt, matching the git-identity precedent in `lib/dotfiles.sh`" — that was wrong, because it didn't check the precedent against `--all`'s actual documented non-interactive guarantee (which `lib/common.sh`'s config-creation prompt already correctly respects). The prompt still always fires in interactive mode (bare `macup`, no `--all`/module args).
